...
export default function TiposEjercicioPage() {
  ...
  const [deleteCandidate, setDeleteCandidate] = useState<TipoEjercicio | null>(
    null,
  );
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) {
        await ejercicioService.actualizarTipoEjercicio(editing.id, {
          nombre,
          descripcion,
        });
      } else {
        await ejercicioService.crearTipoEjercicio({
          key: key.toLowerCase(),
          nombre,
          descripcion,
        });
  ...
  const tiposRecomendados = [
    {
      key: "multiple-choice",
      nombre: "Multiple Choice",
      descripcion: "Ejercicio de selección múltiple",
    },
    {
      key: "true_false",
      nombre: "Verdadero/Falso",
      descripcion: "Ejercicio de verdadero o falso",
    },
    {
      key: "abierta",
      nombre: "Pregunta Abierta",
      descripcion: "Ejercicio de respuesta libre",
    },
    {
      key: "latex",
      nombre: "LaTeX",
      descripcion: "Ejercicio con fórmulas matemáticas",
    },
    {
      key: "programming",
      nombre: "Programación",
      descripcion: "Ejercicio de código con tests automatizados",
    },
  ];
    ...
    <Box sx={{ bgcolor: "#F5DEB3", minHeight: "100vh", py: 4 }}>
      <Container maxWidth="md">
        <Paper sx={{ p: 4 }}>
          ...
          {tiposRecomendados.filter(
            (tr) => !tipos.find((t) => t.key === tr.key),
          ).length > 0 && (
        ...
        >
          ...
          <DialogContent>
            <Typography>
              ¿Eliminar tipo "{deleteCandidate?.nombre}"? Esta acción es
              irreversible.
            </Typography>
          </DialogContent>
          <DialogActions>
