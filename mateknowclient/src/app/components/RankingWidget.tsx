...
export default function RankingWidget({ backendUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000', claseId }: RankingWidgetProps) {
            ...
            const fetchRanking = async () => {
                try {
                    if (payload.type === 'versus') {
                        if (claseId) {
                            const versusData = await actividadService.getClaseVersusRanking(claseId);
                            setVersusRankingData(versusData);
                        } else {
                            const versusData = await actividadService.getGlobalVersusRanking();
                            setVersusRankingData(versusData);
                        }
                            ...
                            <List disablePadding sx={{ pb: 2 }}>
                                    ...
                                    return (
                                        ...
                                        >
                                            <ListItemAvatar sx={{ minWidth: 50 }}>
                                                <Box
                                                    sx={{
                                                        ...
                                                        fontWeight: '900',
                                                        color: isTop3 ? medalColor : theme.palette.text.secondary,
                                                        fontSize: isTop3 ? '1.4rem' : '1.1rem',
                                                        textShadow: isTop3 ? `0 2px 4px ${alpha(medalColor, 0.3)}` : 'none'
                                                    }}
                                                >
                                                    {isTop3 ? (
                                            ...
                                            <ListItemAvatar>
                                                <Avatar
                                                    sx={{
                                                        bgcolor: isTop3 ? alpha(medalColor, 0.1) : alpha(theme.palette.primary.main, 0.1),
                                                        color: isTop3 ? medalColor : theme.palette.primary.main,
                                                        fontWeight: 'bold',
                                                        border: isTop3 ? `2px solid ${medalColor}` : 'none'
                                                    }}
                                                >
                                                    {item.usuario.nombre?.[0] || <PersonRounded />}
                                                </Avatar>
                                            </ListItemAvatar>
                                            ...
                                            <Chip
                                                ...
                                                sx={{
                                                    ...
                                                    minWidth: 80,
                                                    height: 32,
                                                    background: isTop3 ? `linear-gradient(45deg, ${theme.palette.primary.main}, ${theme.palette.primary.dark})` : undefined,
                                                    boxShadow: isTop3 ? '0 4px 12px rgba(0,0,0,0.15)' : 'none'
                                                }}
                                            />
                                        </ListItem>
                                    );
                                ...
                                {rankingData.length === 0 && (
                                    <Box sx={{ p: 6, textAlign: 'center', color: 'text.secondary' }}>
                                        <EmojiEventsRounded sx={{ fontSize: 60, color: 'text.disabled', mb: 2, opacity: 0.5 }} />
                        ...
                        {loading ? (
                            ...
                            <List disablePadding sx={{ pb: 2 }}>
                                ...
                                {versusRankingData.length === 0 && (
                                    <Box sx={{ p: 6, textAlign: 'center', color: 'text.secondary' }}>
                                        <Typography variant="body1">No hay datos de ranking versus aún</Typography>
                                    </Box>
                                )}
                            </List>
                        )}
